...
void worker_thread_func(client_worker_context* ctx, size_t payload_size) try {
  ...
  while (!g_shutdown.load()) {
    ...
    while (!available_send_contexts.empty() &&
      ...
      post_send(ctx->sockets[ctx->next_socket_index.fetch_add(1) % ctx->sockets.size()], send_ctx,
                send_ctx->buffer.data(), total_size, reinterpret_cast<sockaddr*>(&ctx->server_addr),
                ctx->server_addr_len);
    ...
    if (!ex_result) {
      ...
      std::osyncstream(std::cerr) << std::format(
          "[Worker {}] GetQueuedCompletionStatusEx failed with error: {}\n", ctx->processor_id,
          error);
...
int main(int argc, char* argv[]) try {
  ...
  if (gai_err != 0 || res == nullptr) {
    throw std::runtime_error(std::format("getaddrinfo failed for {}:{} with error: {}", server_ip,
  ...
  if (chosen == nullptr) {
    throw std::runtime_error(std::format("No suitable address found for {}:{}", server_ip, port));
  }
